...
class PlanarChamferRecognizer
{
   public:
    // 构造函数
    PlanarChamferRecognizer(std::shared_ptr<AttributeAdjacentGraph> aag,
   ...
   private:
       ...
       void CollectCoaxialCompEdges(
        const std::vector<CompEdge>& compEdges, std::vector<gp_Ax1>& axiss,
        std::vector<std::vector<CompEdge>>& edgeGroups);

       bool CalculateBeltLengthWidth(const std::vector<CompEdge>& compEdges,
                                     std::vector<CompEdge>& springEdges0,
                                     std::vector<CompEdge>& springEdges1, double& beltLength,
                                     double& beltWidth);

       bool CalculateBeltLengthWidthWide(const std::vector<CompEdge>& compEdges,
                                     std::vector<CompEdge>& springEdges0,
                                     std::vector<CompEdge>& springEdges1, double& beltLength,
                                     double& beltWidth);
    ...
    double CalculateEdgeLength(const TopoDS_Edge& edge) const;
       ...
       bool FindNarrowChamfer(const std::vector<CompEdge>& compEdges, ChamferFeature& feature);
       bool FindWideChamfer(const std::vector<CompEdge>& compEdges, ChamferFeature& feature);
